...
type CustomDeploymentLister interface {
	...
	List(selector labels.Selector) (ret []*customdp.CustomDeployment, err error)
...
// List lists all CustomDeployments in the indexer.
func (s *customDeploymentLister) List(selector labels.Selector) (ret []*customdp.CustomDeployment, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*customdp.CustomDeployment))
	})
	return ret, err
}
...
type CustomDeploymentNamespaceLister interface {
	...
	Get(name string) (*customdp.CustomDeployment, error)
...
// List lists all CustomDeployments in the indexer for a given namespace.
func (s customDeploymentNamespaceLister) List(selector labels.Selector) (ret []*customdp.CustomDeployment, err error) {
...
// Get retrieves the CustomDeployment from the indexer for a given namespace and name.
func (s customDeploymentNamespaceLister) Get(name string) (*customdp.CustomDeployment, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(customdp.Resource("customdeployment"), name)
	}
	return obj.(*customdp.CustomDeployment), nil
}
