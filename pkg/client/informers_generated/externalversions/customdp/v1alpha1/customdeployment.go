...
import (
	customdp_v1alpha1 "api-server/pkg/apis/customdp/v1alpha1"
	...
	v1alpha1 "api-server/pkg/client/listers_generated/customdp/v1alpha1"
...
func NewCustomDeploymentInformer(client clientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCustomDeploymentInformer(client clientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.CustomdpV1alpha1().CustomDeployments(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.CustomdpV1alpha1().CustomDeployments(namespace).Watch(options)
			},
		},
		&customdp_v1alpha1.CustomDeployment{},
		resyncPeriod,
		indexers,
	)
}

func (f *customDeploymentInformer) defaultInformer(client clientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *customDeploymentInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&customdp_v1alpha1.CustomDeployment{}, f.defaultInformer)
}
